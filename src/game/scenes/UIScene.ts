...
export class UIScene extends Phaser.Scene {
  ...
  private async openLeaderboardOverlay(): Promise<void> {
    ...
    backdrop.fillRect(0, 0, W, H);
    backdrop.setAlpha(0);
    this.tweens.add({ targets: backdrop, alpha: 0.88, duration: UI.animation.fadeScene, ease: 'Sine.easeOut' });
    ...
    const tabGap = Math.round(4 * localDpr);
    const tabW   = Math.floor((pW - tabGap * 3 - 16) / 4);
    const tabsY  = titleY + titleFontSize / 2 + 10 + tabH / 2;

    const tabHandles = new Map<Difficulty, ButtonHandle>();

    // Separator + table area
    const sepY        = tabsY + tabH / 2 + 10;
    const tableStartY = sepY + 14;
    ...
    // Table render function — clears and refills tableContainer
    ...
    difficulties.forEach((diff, i) => {
      const tx     = -pW / 2 + 8 + i * (tabW + tabGap) + tabW / 2;
      const handle = createButton(this, {
        ...
        fixedWidth:  tabW,
        fixedHeight: tabH,
        fontSize:    Math.round(7 * localDpr),
      });
